...
router.get('/',async (ctx)=>{
    ...
    var pageSize=8;
    var result= await DB.find('classes',{},{},{
        page,
        pageSize
    });
    var count= await  DB.count('classes',{});  /*总数量*/
    await  ctx.render('admin/classes/list',{
        list:result,
        page:page,
...
router.get('/add',async (ctx)=>{
    ...
    await  ctx.render('admin/classes/add');
...
router.get('/edit',async (ctx)=>{
     ...
     * */
    var id=ctx.query.id;
    var result=await DB.find("classes",{"_id":DB.getObjectId(id)});
    console.log(result);

    await  ctx.render('admin/classes/edit',{
        list:result[0],
...
router.post('/doEdit',tools.multer().single('pic'),async (ctx)=>{
    ...
    //更新数据库
    await  DB.update('classes',{'_id':DB.getObjectId(id)},json);
